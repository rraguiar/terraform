locals {
  ...
  load_balancer_configs = (
    ...
    [{
      ...
      sidecar_port   = var.sidecar_port_mappings[0].containerPort
    }]
  )
...
resource "aws_ecs_task_definition" "this" {
  ...
  container_definitions = "[${module.sidecar_container.json_map_encoded},${module.main_container.json_map_encoded}]"
}
...
module "main_container" {
  ...
  container_memory_reservation = var.container_memory_mib
  container_cpu                = var.container_cpu_units
...
module "sidecar_container" {
  ...
  container_image   = var.sidecar_image
  container_name    = var.sidecar_name
  command           = var.sidecar_command
  environment       = var.sidecar_environment
  secrets           = var.sidecar_secrets
  working_directory = var.sidecar_working_directory

  links = [ var.container_name ]

  container_memory_reservation = var.sidecar_memory_mib
  container_cpu                = var.sidecar_cpu_units
  container_depends_on         = var.sidecar_depends_on

  port_mappings = var.sidecar_port_mappings
  mount_points  = local.sidecar_mount_points
  ...
  linux_parameters        = var.sidecar_linux_parameters
  docker_security_options = var.sidecar_docker_security_options
  environment_files       = var.sidecar_s3_environment_files
}
...
resource "aws_ecs_service" "this" {
  ...
  dynamic "load_balancer" {
    ...
    content {
      ...
      container_name   = lbc.value.sidecar_name
      container_port   = lbc.value.sidecar_port
    }
  }
